...
public class Geosphere {
    ...
    private void calcTriIndices(int[] aux) {
      ...
      for (int row = 0; row < (level + 1); row++) {
        ...
        for (int tri = 0; tri < trisOnRow; tri++) {
          int offset = tri / 2;
          if (tri % 2 == 0) {
            triangles[triIndex++] = new int[] { aux[a[0] + offset], aux[a[1] + offset],
                aux[a[2] + offset] };
          } else {
            triangles[triIndex++] = new int[] { aux[b[0] + offset], aux[b[1] + offset],
                aux[b[2] + offset] };
          }
        }
  ...
  public Geosphere(float radius, int level) {
    this.radius = radius;
    this.level = level;
    new Builder().build();
  }
