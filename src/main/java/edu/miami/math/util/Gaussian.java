...
/**
 * Gaussian blur parameters (for image processing)
 * 
 * @author justin
 */
public class Gaussian {
  public static class BlurParams {
    ...
    public BlurParams(Vec2[] offsets, float[] weights) {
  ...
  private static float gaussian(int n, float blurriness) {
    return (float) ((1.0f / Math.sqrt(2 * Math.PI * blurriness)) * Math.exp(-(n * n)
        / (2 * blurriness * blurriness)));
  }
  ...
  private static BlurParams calcBlurParams(float x, float y, int samples, float blurriness) {
    ...
    Vec2[] offsets = new Vec2[samples];

    // the starting pixel
    weights[0] = gaussian(0, blurriness);
    offsets[0] = new Vec2(0);
    ...
    for (int i = 0; i < samples / 2; i++) {
      ...
      Vec2 offset = new Vec2(x, y).times(i * 2 + 1.5f);
