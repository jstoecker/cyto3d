...
/**
 * Divides space up into axis-aligned bounding boxes that contain triangles. Note that multiple
 * nodes can contain a triangle, so each triangle is flagged when it is rendered or used in a
 * calculation. These flagged triangles are stored in a hash set which is cleared before the octree
 * is traversed.
 * 
...
public class Octree {
  private class Node {
    ...
    private void split(List<Triangle> triangles) {
      children = new Node[8];
      Vec3 toC = bounds.getDiag().over(2.0f);
      for (int i = 0; i < 8; i++) {
        // split the current node's box into the child's octant
        int j = (i % 4);
        Vec3 o = new Vec3(j == 1 || j == 2 ? toC.x : 0, i > 3 ? toC.y : 0, j > 1 ? toC.z : 0);
        Vec3 cnMin = bounds.getMin().plus(o);
    ...
    private List<Triangle> checkTriangles(BoundingBox box, List<Triangle> triangles) {
      ...
      for (int i = 0; i < triangles.size(); i++)
        if (box.intersects(triangles.get(i))) contained.add(triangles.get(i));
      return contained;
    }
    ...
    public Vec3 intersect(Ray r) {
      ...
      if (bounds.intersect(r) == null) return null;

      if (children == null) {
        // this is a leaf node, so check the triangles
        List<Vec3> hits = new ArrayList<Vec3>();
        for (int i = 0; i < triangles.size(); i++) {
          // if the triangle is marked, ignore it; otherwise, mark it
          if (checked.contains(triangles.get(i))) continue;
          checked.add(triangles.get(i));

          Vec3 x = triangles.get(i).intersect(r);
          if (x != null) hits.add(x);
        }

        return GMath.getNearest(r.getPosition(), hits);
      } else {
        // not a leaf, so check children
        List<Vec3> hits = new ArrayList<Vec3>();
        for (int i = 0; i < 8; i++) {
          Vec3 x = children[i].intersect(r);
          if (x != null) hits.add(x);
        }
        return GMath.getNearest(r.getPosition(), hits);
      }
   ...
   * 
   * @param triangles - the set of triangles used to divide the space
   * @param maxNodeSize - the number of triangles a node can contain before it is split; must be
   *          greater than 0
   */
  ...
  /**
   * Returns the closest ray-triangle intersection point, or null if there is no intersection
   */
  public Vec3 intersect(Ray r) {
    checked.clear();
    Vec3 x = root.intersect(r);
