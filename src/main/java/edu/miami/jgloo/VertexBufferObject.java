...
public class VertexBufferObject implements GLDisposable {
   ...
   * 
   * @param data - data to be stored in VBO
   * @param dataSize - total size in bytes of the data
   * @param target - the type of data the vertex buffer handles:<br>
   *          GL_ARRAY_BUFFER_ARB - vertex, normal, tex. coords, etc.<br>
   *          GL_ELEMENT_ARRAY_BUFFER_ARB - indexing data<br>
   * @param usage - determines how the VBO will be used for efficiency:<br>
   ...
   * @param dataSize - size of the buffered data in bytes
   * @param offset - the starting location for copying data
   */
  ...
  public void finalize() {
    if (!disposed) System.err.printf("WARNING: VBO %d was not disposed!\n", id);
  }
