...
/**
 * Display list rendering for static mesh geometry. A display list is a compiled set of commands
 * that is cached for future use. For systems that don't support vertex buffer objects, this is a
 * good alternative.
 * 
      ...
      for (MeshPart part : mesh.parts) {
        ...
        for (MeshFace face : faces) {
          ...
          for (int i = 0; i < indices.length; i++) {
            ...
            if (vn != null) gl.glNormal3fv(vn, 0);
            if (vt != null) gl.glTexCoord2fv(vt, 0);
            gl.glVertex3fv(vp, 0);
          }
        }
        gl.glEnd();
