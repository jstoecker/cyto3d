...
/**
 * Imports OBJ model as a standard Mesh object. Resources can be loaded from within jar file by
 * setting classLoader.
 * 
...
public class ObjMeshImporter {
  ...
  public Mesh loadMesh(BufferedReader br) throws IOException {
    ...
    while ((line = br.readLine()) != null) {
      ...
      else if (line.startsWith("usemtl ")) changeMaterial(line);
    }

    // remove any empty parts
    for (int i = 0; i < mesh.getParts().size(); i++) {
      if (mesh.getParts().get(i).getFaces().size() == 0) mesh.getParts().remove(i);
    }
  ...
  private void initDataStructures() {
    ...
    min = new Vec3(Float.POSITIVE_INFINITY);
    max = new Vec3(Float.NEGATIVE_INFINITY);
    ...
    // the .obj files
    if (materialPath == null) materialPath = modelPath;
    if (texturePath == null) texturePath = modelPath;
  ...
  private void readVertexPosition(String line) {
    ...
    if (vertexPosition.length > 2) {
      if (vertexPosition[0] > max.x) max.x = vertexPosition[0];
      if (vertexPosition[1] > max.y) max.y = vertexPosition[1];
      if (vertexPosition[2] > max.z) max.z = vertexPosition[2];
      if (vertexPosition[0] < min.x) min.x = vertexPosition[0];
      if (vertexPosition[1] < min.y) min.y = vertexPosition[1];
      if (vertexPosition[2] < min.z) min.z = vertexPosition[2];
    }
