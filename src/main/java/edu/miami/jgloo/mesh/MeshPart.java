...
public class MeshPart implements GLDisposable {
  ...
  /**
   * Checks if face contains indices with values exceeding 16 bits; if so, switches index type to
   * unsigned int (32 bit).
   */
  private void checkIndexSize(MeshFace face) {
    ...
    if (indexType == GL.GL_UNSIGNED_SHORT) {
      ...
      for (int i = 0; i < indices.length; i++)
        if (indices[i] > USHORT_MAX) indexType = GL2ES2.GL_UNSIGNED_INT;
    }
  }
  ...
  public void dispose(GL gl) {
    if (material != null) material.dispose(gl);
    disposed = true;
  }
