...
public class ObjExporter {
  ...
  public static class Data {
     ...
     * 
     * @param vertices - Vertex position coordinates. Each vertex position should be an array such
     *          that vertices.length is the number of vertices in the model; vertices[i][j] refers
     *          to the j-th component of the i-th vertex.
     * @param normals - Vertex normal vector components. Each vertex normal should be an array such
     *          that normals.length is the number of normal vectors in the model; normals[i][j]
     *          refers to the j-th component of the i-th normal vector. If set to null, no normals
     *          are used.
     * @param texCoords - Vertex texture coordinates. Each texture coordinate should be an array
     *          such that texCoords.length is the number of vertices in the model; texCoords[i]
     *          refers to the (u,v) coordinate pair for the i-th vertex. If set to null, no texture
     *          coordinates are used.
     * @param faces - Face vertex indices. Each face should be an array of the indices of the
     *          vertices for that face such that faces.length is the number of faces in the model;
     *          faces[i][j] refers to the j-th vertex index for the i-th face.
     * @param faceType - Primitive type for faces: triangle list, triangle fan, quads, etc.
     * @param zeroIndexed - Should be set to true if vertex indices (provided in faces parameters)
     *          begin with 0. Obj format starts indexing at 1, so this should be set to false if the
     *          indices do not start at 0.
     */
    public Data(float[][] vertices, float[][] normals, float[][] texCoords, int[][] faces,
        FaceType faceType, boolean zeroIndexed) {
      ...
      if (zeroIndexed) for (int i = 0; i < faces.length; i++)
  ...
  private class DataWriteThread extends Thread {
    ...
    private void writeToFile(BufferedWriter out) throws IOException {
      ...
      if (useTexCoords) for (int i = 0; i < data.texCoords.length; i++)
        writeTexCoord(data.texCoords[i], out);

      if (useNormals) for (int i = 0; i < data.normals.length; i++)
  ...
  private static void writeFace(int[] f, boolean texCoords, boolean normals,
      Data.FaceType faceType, BufferedWriter out) throws IOException {
