...
/**
 * Checks the OpenGL / shading language versions and queries which extensions are available.
 * 
...
public class GLInfo {
  ...
  public GLInfo(GL gl) {
    ...
    if (gl_version != null) glVersion = Float.parseFloat(gl_version.substring(0, 3));
    if (sl_version != null) slVersion = Float.parseFloat(sl_version.substring(0, 3));

    extensions = gl.glGetString(GL.GL_EXTENSIONS);
  }

  /**
   * Checks if a particular extension is supported by the client's OpenGL implementation.
   */
  ...
  /**
   * Prints out basic info about client's OpenGL implementation. This requires that the client info
   * has been checked by calling GLInfo.check(GL gl).
   */
  public void print() {
